...
public class BombingRun extends Explosive {
     ...
     */
    @Override
    public void checkCollision(int x, int y, int width, int height, Obstacle obstacle) {
    ...
    public void checkHit(Tank tank) {
        if (hasCollision(tank.getX(), tank.getY(), tank.getIcon().getWidth() * tank.getScale(), tank.getIcon().getHeight() * tank.getScale()) && hit && !tank.getShield() && !tank.getGhost())
