...
public class ObstacleGenerator {
     ...
     */
    public static List<Obstacle> generateObstacles(List<Tile> tiles, String fileName, int health) {
            ...
            for (int i = 0; i < lineCount; i++) {
                for (int j = 0; j < lineLength; j++) {
                    ...
                    if (i % 2 == 0 && j % 2 == 0)     // Corner
                        obstacle = new Corner((j / 2) * scale + xStart, (i / 2) * scale + yStart, smallScale, smallScale, cornerColor, false, health);
                    else if (i % 2 == 0) {            // Horizontal wall
                        if (random <= 5 && map[i][j].equals("0"))
                            obstacles.add(new Fence((j / 2) * scale + smallGap + xStart, (i / 2) * scale + yStart, largeScale, smallScale, wallColor, true, health));
                        obstacle = new HorizontalWall((j / 2) * scale + smallGap + xStart, (i / 2) * scale + yStart, largeScale, smallScale, wallColor, false, health);
                    } else if (j % 2 == 0) {          // Vertical wall
                        if (random <= 5 && map[i][j].equals("0"))
                            obstacles.add(new Fence((j / 2) * scale + xStart, (i / 2) * scale + smallGap + yStart, smallScale, largeScale, wallColor, true, health));
                        else
                            obstacle = new VerticalWall((j / 2) * scale + xStart, (i / 2) * scale + smallGap + yStart, smallScale, largeScale, wallColor, false, health);
                    } else                           // Tiles
                        tiles.add(new Tile(i, j, (j / 2) * scale + smallGap + xStart, (i / 2) * scale + smallGap + yStart, largeScale, largeScale, true, health));
